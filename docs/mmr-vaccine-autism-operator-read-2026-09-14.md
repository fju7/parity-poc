# mmr-vaccine-autism — operator read, 2026-09-15 (final, record a42989d)

One page, for the human read the design requires (§6a) before any flip. Record `20260915T210829+0000-a42989d`, gate `a42989d`, stored in `topic_publications` and served to the draft page. This is the build finished: CHRONOLOGY, the publish gate on already-changed sources, the erratum precision check, the two retracted markers, the corpus corrections 081–083 and 090–091, the six refused plain summaries (088–089), the word-form figure rule, three hand-fetched documents admitted under `OPERATOR_SUPPLIED`, and the date-only levelling fix.

**33 of 37 sources survive · 34 claims figure-bound · 1 quotation-bound · 72 source-confirmed only (56%) · 21 withheld.** Status of survivors at freeze: Wakefield 1998 ×2 retracted (subject-only on the page), Jain 2015 ×2 corrected (JAMA erratum), the Lancet retraction notice (a `retraction_notice` about Wakefield 1998), 7 documents with no status registry (6 agency/payer pages and the Walker-Smith judgment), 21 unchanged. **Three survivors were fetched by a person, not the machine**, and the record says so on each: the Lancet retraction notice (sha256 `b70a2200…`), Deer's BMJ investigation (`79373a9f…`) and Godlee's BMJ editorial (`f58bdbdd…`), all supplied by Fred Ugast on 2026-09-15 after the registry's title was found in the file.

**Read the figure-bound count as a correction, not a loss.** The previous record said 64 figure-bound; this one says 34 plus one quotation. **The 30-point drop is the fix working, not a regression.** Twenty-nine of the 64 had been bound on a *year alone* — "the Smeeth 2004 study found…" — where CHRONOLOGY confirmed the year and the claim stated no other figure; the gate now levels those as source-confirmed, which is what they are. The thirtieth is 'an elaborate fraud', now labelled quotation-bound rather than figure-bound. No claim that was shown is now withheld; no claim that was withheld is now shown. What the page shows is identical; what it calls each thing is now accurate.

**The 29, swept individually** (does the source's fetched text support anything in the claim beyond the year?): **24 yes** — the paraphrased finding is in the abstract (Taylor 1999 ×5, Honda ×5, Taylor 2002 ×3, Jain ×2, Madsen "age/time/date of vaccination", Fombonne ×2, Hviid ×2, Smeeth "consistent across sensitivity analyses", the FDA 1999 review, the February 2010 retraction). **5 no** — the source's fetched text carries nothing in the claim beyond identity and year:
- `19214753` "paid by lawyers … 1998" on the retraction notice — a **false bind**; the notice never mentions lawyers or payment. **Re-cited to Deer (migration 091)**; now source-confirmed on Deer, whose text reports the payment.
- `f0b091fa` "Smeeth 2004 … no evidence of temporal clustering" — Smeeth's abstract has no temporal-clustering analysis (that finding is Taylor 1999's). Not re-cited; the operator decides.
- `b8b27c3b` "Madsen 2002 … no dose-response relationship" — the abstract compares vaccinated with unvaccinated and says nothing about doses. Not re-cited.
- `6bbf8769` "Hviid 2019 … high-risk subgroups including children with preterm birth or low birth weight" — the abstract says "autism risk factors (based on a disease risk score)"; neither preterm nor birth weight appears. Not re-cited.
- `62c10907` "The Cochrane systematic review … published in 2020, is described as the most comprehensive … to date" — the source is pub5 (2021); "described as" by no one in the text. Not re-cited.
All five render source-confirmed ("wording not machine-checked"), which is the honest label; whether the four stay is the operator's call.

> Records before this one today: `4fefcb5` (33 of 37, 64/43/21 — 090), `0e25a32` (31 of 35 — the notice and Deer admitted), `22387c7` (29 of 35 — the word-form fix), `b902246` (26 of 35 — Europe PMC stopped answering; kept, not signed).
...
- **Added the fourth sentence (the operator's instruction, 2026-09-15):** the BMJ's correction bmj.d1678 to Godlee's editorial. Crossref reports c7452 `unchanged`, so nothing machine-side would ever have surfaced it; a reader who finds it will ask why the page did not. It enters the corpus by **migration 092 (authored, awaiting review)** — the correction as its own source, titled "Correction — …" so the Sources panel cannot show two identical titles, and the editorial's row carrying `corrected_by`. Until 092 is applied and the page reads a hand-curated correction as a status marker, the sentence is ahead of the record: **do not ratify it before 092.**
...
## 2. Thirteen IDENTITY_ONLY claims, weakest first (of 72 — 29 of them relabelled from figure-bound today, listed in the box above) — each will carry *"Source confirmed; wording not machine-checked"*
