...
| A claim with no figure and no quotation is checked against its source's subject (SUBJECT: named bodies/people, prose quantities, distinguishing words vs the held text); shown as "Checked against the source's subject, not its wording" when found, withheld when not; the header counts SUBJECT_BOUND of shown; the page names the check's blind spot (enumeration vs plurality word) | `verify/subject.py bind_subject`; `verify/publish.py` (`SHOWN`, `SUBJECT_BOUND`); `RecordMarkers.jsx` (`support === "SUBJECT_BOUND"`, `TopicLine`) |
| No page-level aggregate of support levels; the header names the freeze date and the status counts and points to the per-claim markers ("Every claim shown says what was checked against its source."); the distribution is stated in prose on the methodology page | `RecordMarkers.jsx TopicLine`; `MethodologyView.jsx` "No page-level score" |
| Nothing is shown on the document's identity alone (IDENTITY_ONLY withheld since 2026-09-15) | `verify/publish.py SHOWN`; `publication_row` |
