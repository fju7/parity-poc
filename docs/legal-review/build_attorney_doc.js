...
// ---------------------------------------------------------------------------
// PROVISION SHEETS
// CC: replace this array with the twelve rows from
// backend/data/verify/candidates.json + the retrieved provision text.
// One example entry below shows the required shape. Do not paraphrase the
// statutory text — paste the retrieved excerpt verbatim.
// ---------------------------------------------------------------------------
const PROVISIONS = [
  {
    id: 'OH-01',
    denialCode: 'CO-16',
    denialMeaning: 'Claim/service lacks information or has submission/billing error(s).',
    payerType: 'Commercial (non-Medicare)',
    citation: 'Ohio Rev. Code § 3901.381',
    heading: '[official section heading, as retrieved]',
    excerpt: '[verbatim excerpt of the retrieved provision — the paragraphs the assertion relies on]',
    assertion: '[the exact sentence the letter would write using this provision]',
    rationale: '[why this provision was selected for this denial code and payer type]',
    ourNote: 'Our automated check confirms this provision exists, that its text contains the figures asserted, and that it applies to a commercial payer in Ohio. It cannot confirm that it is the right provision to cite for this denial reason. That is the question on this sheet.',
  },
];
// ---------------------------------------------------------------------------
...
Packer.toBuffer(doc).then(buf => {
  fs.writeFileSync('/mnt/user-data/outputs/CivicScale-Legal-Review-Request.docx', buf);
