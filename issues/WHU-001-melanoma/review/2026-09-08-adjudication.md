...
# Republication, 10 September 2026

The change-log entry of 9 September claimed that publishing the corrections
recorded above meant binding every sentence on this page to the words it rests
on. It was 131 of 343. The correction is on the page and in `corrections.md`; the
false claim is what this republication exists to replace.

### What remains open, at the moment of this acceptance

*Derived from the checks' own outputs. Not assembled from memory — see `open_list.py` for why that distinction is the whole point.*

**Coverage.** Rules 1 and 2 examined **131** of this page's **343** body sentences. **212 were not examined** — a binding row is created only where an anchor is detectable, so a claim carrying no figure, quotation, named trial or registry identifier never enters either rule. **Not examined is not failing:** nothing in those 212 has been shown wrong, and they may be entirely sound.

**Claims about what we know.** 1 of 52 sentences saying what we hold or have read could be tied to a single source and checked against the store; the rest are reported as not evaluated rather than passed.

**Checks not returning ok (6):**

- **sentences resting on a human attestation** — warn: 2 of 131 rest on a document no check may read: a named person read it, the record of that reading is named, and the place in the document is named. This is not a verified span and is never counted as one.
- **correction lookups that failed** — warn: 2 source(s) could not be looked up and are NOT known to be clean: S007, S030
- **sources with no resolvable identifier** — warn: 15 source(s) carry no DOI, PMID, PMCID or PII in the URL we hold, so no automatic check is possible: S001, S002, S005, S006, S008, S009, S010, S011
- **corrections not yet read** — BLOCKED: 1 source(s) have been formally corrected and we do not hold the correction: S029: Erratum in 31442371 — until it is read we do not know whether it touches a figure on this page
- **documents a substance test examined** — warn: 23 of 28 held document(s) — 7 by the article test, 16 by the page test. 5 were checked only against a length floor, which is a weaker thing
- **documents never round-tripped** — warn: 10 held document(s) have no canary result: S014, S017, S022, S023, S024, S025

**Waived at a previous publication (1):**

- 2026-09-09T18:33:15 — S029 ONLY. The 2018 NEJM erratum (PMID 31442371, DOI 10.1056/NEJMx180040), a single notice covering thirteen articles, applies to Eggermont et al. EORTC 18071. We have not read it and cannot: NEJM returns 403 to a script, Europe PMC records it as neither open access nor in its archive, and neither PubMed nor Crossref carries an abstract. Operator ruling 2026-09-09: publish. Grounds — (1) the page discloses the gap to the reader in the S029 source note, naming the erratum and saying we have not read it and do not know whether it touches a figure quoted here; (2) the claim that rests on this paper is that one adjuvant checkpoint inhibitor reported an OS benefit against placebo as a prespecified secondary endpoint at a dose and toxicity that kept it from standard care, which survives any decimal-level correction and would fall only to a retraction, and retractions are not issued as one line in a thirteen-paper notice; (3) removing a true, well-sourced counterexample on the strength of an unread notice would restore the impression OR-008 exists to correct. Corroboration, labelled as corroboration and not as closure: every figure quoted from this paper appears in the PubMed record of PMID 27717298 retrieved 2026-09-09, which post-dates the erratum. S029 keeps an open, undispositioned errata item. If the notice is ever obtained and touches a figure on this page, that is a correction and goes in the log. This waive covers S029 and nothing else.

### How this list was produced, which is not part of the attestation

**Six of the eight items above would not have appeared on a list written by
hand.** The list assembled on 9 September said one thing — the S029 erratum. This
one is generated by `open_list.py` from the checks' own outputs, and it adds: the
212 sentences neither rule examined; two correction lookups that failed and are
reported as NOT known to be clean rather than as clean; fifteen sources carrying
no resolvable identifier; five held documents checked only against a length
floor; ten never round-tripped by the canary; and the two sentences whose support
is a named person's word, quoted rather than counted.

None of those was concealed. Several were printing in gate output at the time.
**Nobody was assembling them**, and rule 13 places its entire weight on the
operator knowing what remains open. That is a fact about the mechanism and it is
recorded here rather than in the attestation below, because it is not something
the operator is attesting to — it is something the apparatus owed him and did not
supply until today.

**One limit on the verification, stated rather than left to be noticed.** The
record named below was performed against sha `7ae5304c…`. The page has changed
since — yesterday's publication and today's change-log correction — so that
record does not cover the current bytes. `confirm-review` is what carries it
forward, and it is run before this publishes.

## ROUND-2026-09-10

The change set covering everything between the review snapshot `a788f00a…` and
the page this republication puts out. Two decisions stand behind it and both are
in this directory:

- **ROUND-2026-09-08** — the 8 September outside review, its adjudication, the
  remediation order and the rulings that closed it. That set covered
  `a788f00a… → 2727429064d6…`, the sha published on 9 September.
- **The 10 September 2026 rulings** — `2026-09-10-the-140-ruling.md`,
  `2026-09-10-melanoma-coverage-ruling.md` and
  `2026-09-10-changelog-correction-ruling.md`, which established that rules 1 and
  2 examined 131 of 343 sentences, that the change-log entry claiming *every*
  sentence was bound was therefore false, and that it be corrected rather than
  deleted.

Recorded as a set rather than per sentence because the reasoning is in those
...
that did not author this adjudication.

I have not independently verified the technical content and am not attesting to
it. I am recording a publication decision: this issue publishes under this
publication's name with the items above open — including 212 of its 343 sentences
not examined by rules 1 and 2, and the S029 erratum we have not read and which
the page discloses to the reader.

The open list above was derived from the checks' own outputs, not assembled from
memory.

Accepted by Fred Ugast, 10 September 2026.
