...
function EmployerDashboardInner() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [subRes, claimsRes, usersRes] = await Promise.all([
  ...
  const quickActions = [
    ...
    { label: "Account & Billing", desc: "Manage subscription and team", path: "/billing/employer/account", icon: "\u2699\uFE0F" },
  ];
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ display: "flex", gap: 16, marginBottom: 32, flexWrap: "wrap" }}>
          <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)",
            ...
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.4)", marginBottom: 4 }}>Plan</div>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ background: tierColor, color: "white", fontSize: 12, fontWeight: 700,
                             padding: "2px 10px", borderRadius: 20 }}>{tierLabel}</span>
            </div>
          </div>
          <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)",
                        borderRadius: 12, padding: "20px 24px", flex: 1, minWidth: 180 }}>
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.4)", marginBottom: 4 }}>Status</div>
            <div style={{ fontSize: 18, fontWeight: 600, color: subscription?.status === "active" ? "#10B981" : "#EF4444" }}>
              {subscription?.status === "active" ? "Active" : "Inactive"}
            </div>
          </div>
          <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)",
                        borderRadius: 12, padding: "20px 24px", flex: 1, minWidth: 180 }}>
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.4)", marginBottom: 4 }}>Renewal</div>
