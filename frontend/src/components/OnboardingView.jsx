...
export default function OnboardingView({
  ...
  // Employer code state
  const [employerCode, setEmployerCode] = useState(initialData?.employerCode || "");
  const [employerCompany, setEmployerCompany] = useState(initialData?.employerCompany || "");
  const [employerCodeStatus, setEmployerCodeStatus] = useState(
    initialData?.employerCode ? "linked" : "idle"
  ...
  const handleVerifyEmployerCode = async () => {
    if (!employerCode.trim()) return;
    setEmployerCodeStatus("verifying");
    try {
      const res = await fetch(`${API_BASE}/api/employer/verify-code`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ code: employerCode.trim() }),
      });
      const data = await res.json();
      if (data.valid) {
        setEmployerCompany(data.company_name);
        setEmployerCodeStatus("linked");
        // Save to profile
        if (session?.user) {
          await supabase.from("profiles").update({ employer_code: employerCode.trim() }).eq("id", session.user.id);
        }
      } else {
        setEmployerCodeStatus("error");
      }
    } catch {
      setEmployerCodeStatus("error");
    }
  ...
  const handleRemoveEmployerCode = async () => {
    setEmployerCode("");
    setEmployerCompany("");
    setEmployerCodeStatus("idle");
    if (session?.user) {
      await supabase.from("profiles").update({ employer_code: null }).eq("id", session.user.id);
    }
  };
        ...
        {isAccountView && (
          <div className="mt-10 pt-8 border-t border-gray-200">
            ...
            <div className="space-y-4">
              ...
              {/* Employer code subsection — shown when employer toggle is on */}
              {employerEnabled && (
                <div className="p-4 border border-gray-200 rounded-xl bg-gray-50">
                  <p className="font-medium text-[#1B3A5C] text-sm mb-1">Employer Dashboard Code</p>
                  <p className="text-xs text-gray-500 mb-3">
                    Enter the code from your employer to link your anonymous billing data to their benefits dashboard.
                  </p>

                  {employerCodeStatus === "linked" ? (
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-[#0D7377]">
                          Linked to {employerCompany}
                        </span>
                      </div>
                      <button
                        type="button"
                        onClick={handleRemoveEmployerCode}
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (employerCodeStatus === "error") setEmployerCodeStatus("idle");
                          }}
                          placeholder="e.g. ACME-2847"
                          className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-800 placeholder:text-gray-400 focus:outline-none focus:border-[#0D7377] focus:ring-1 focus:ring-[#0D7377]"
                        />
                        <button
                          type="button"
                          onClick={handleVerifyEmployerCode}
                          disabled={!employerCode.trim() || employerCodeStatus === "verifying"}
                          className="px-4 py-2 text-sm font-medium text-white bg-[#0D7377] rounded-lg hover:bg-[#0B6164] cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          {employerCodeStatus === "verifying" ? "Checking..." : "Verify & Save"}
                        </button>
                      </div>
                      {employerCodeStatus === "error" && (
                        <p className="text-xs text-red-600 mt-2">Code not recognized. Please check with your employer.</p>
                      )}
                    </>
                  )}
                </div>
