...
export default function EmployerContractParser() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const storedSession = localStorage.getItem("employer_session");
      if (storedSession) {
        try { const s = JSON.parse(storedSession); if (s.email) formData.append("email", s.email); } catch {}
      }
        ...
        {view === "error" && error === "subscription_required" && (
          <div style={{ textAlign: "center", paddingTop: "60px" }}>
            ...
            <p style={{ fontSize: "15px", color: "#94a3b8", lineHeight: "1.7", maxWidth: "480px", margin: "0 auto 24px" }}>
              The contract parser is available with an active Parity Employer subscription. Subscribe to unlock contract analysis, unlimited claims checks, RBP calculator, and trend monitoring.
            </p>
