      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1px 1fr", gap: "20px" }}>
        ...
        <div>
          ...
          <button
            onClick={() => {
              window.print();
              const subject = `CivicScale Benchmark Report${benchmarkData.industry ? ` — ${benchmarkData.industry}` : ""}`;
              const body = [
                "Here is a summary of our CivicScale benchmark analysis:",
                "",
                benchmarkData.industry ? `Industry: ${benchmarkData.industry}` : null,
                benchmarkData.company_size ? `Company size: ${benchmarkData.company_size}` : null,
                benchmarkData.state ? `State: ${benchmarkData.state}` : null,
                benchmarkData.pepm ? `PEPM: $${benchmarkData.pepm}` : null,
                benchmarkData.percentile ? `Percentile: ${benchmarkData.percentile}` : null,
                benchmarkData.annual_gap ? `Annual gap: $${Number(benchmarkData.annual_gap).toLocaleString()}` : null,
                "",
                "Full report attached as PDF (use browser print to save).",
                "",
                "Generated by CivicScale — https://civicscale.ai",
              ].filter(Boolean).join("\n");
