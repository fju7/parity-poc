...
- THE POSTURE CHECK (supersedes the widened policy query): backend/verify/db_posture.py
  enumerates every TABLE from pg_class via public.db_posture() (SECURITY
  DEFINER, service_role execute; migration 087) and lands each in one bucket:
  service_role_only | public_read (data/verify/db_posture_allow.json, with a
  reason) | gated_read (078's status='published') | user_scoped (auth.uid())
  | FAIL. tests/verify/test_db_posture.py asserts zero FAIL live and runs in
  CI (.github/workflows/db-posture.yml: every push touching migrations, and
  daily -- a policy can change from the dashboard with no commit). It found
  what the policy query could not: provider_appeal_letters had RLS on, NO
  policy, and full anon DML grants -- closed by default-deny only.
  `python -m verify.db_posture` from backend/ prints the buckets.
- RULE: A POLICY NAME IS A CLAIM; THE QUAL IS THE BEHAVIOUR. A name that
  describes a condition ("by share token", "own", "authenticated users",
  "service role") must contain that condition in its qual/with_check; the
  posture check fails any that does not (name_claims_condition_qual_is_true).
  Three of three on 2026-09-15 were false claims.
- RULE: A COMPATIBILITY SHIM THAT DEGRADES GRACEFULLY IS A SWALLOW.
  _attach_verification_column caught a missing column and carried on without
  the verdict, one print per process. Graceful degradation and silent data
  loss are the same code path seen from two directions; when the degraded
  path drops a field the record is supposed to carry, it fails loudly instead.
- TRUNCATE IS NOT SUBJECT TO RLS. anon/authenticated held it on every table
  CREATE TABLE made. 087 revoked TRUNCATE/REFERENCES/TRIGGER from clients on
  all 88 tables and every DML no admitting policy needs (137 statements,
  generated by verify.db_posture.hygiene_statements and pinned by test to
  the migration file). Supabase default privileges will re-grant full DML
  to anon/authenticated on any NEW table: the posture check catches it; the
  migration that creates the table must REVOKE, as the grant-hygiene rule
  already says.
- 087 APPLIED 2026-09-15 ~15:20Z: db_posture(); broker_client_benchmarks
  "public read by share token" (no token in the qual) -> service-role-only,
  the token check lives in GET /api/employer/shared-report/{token}; the three
  "Allow anonymous inserts" policies dropped (backend writes with the service
  key; frontend never inserts); signal_events authenticated INSERT dropped
  (routers/signal_events.py writes with the service key). Probed after: anon
  GET/INSERT on all five -> 401; Signal gated reads and the shared report
