...
Runs all eight pipeline steps in sequence with a single command:
  1. collect_sources.py          — Load sources into Supabase
  2. extract_claims.py           — Extract + deduplicate claims
  3. classify_claims.py          — Classify claim_type + consensus_type
  4. score_claims.py             — Score claims across 6 dimensions
  5. map_consensus.py            — Map consensus per category
  6. generate_summary.py         — Generate versioned summary
  7. detect_changes.py           — Detect evidence changes vs previous run
  8. generate_notifications.py   — Generate notifications for subscribers
...
def main():
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Pass --force to steps 2-5 (clear and re-run)",
    )
    parser.add_argument(
        "--from",
        type=int,
    ...
    parser.add_argument(
        ...
        dest="to_step",
        metavar="STEP",
        help="Stop after step N (1-8, default: 8)",
    )
    parser.add_argument(
        "--issue-slug",
        type=str,
    ...
    if args.from_step < 1 or args.from_step > 8:
        print(f"ERROR: --from must be between 1 and 8 (got {args.from_step})")
        sys.exit(1)
    if args.to_step < 1 or args.to_step > 8:
        print(f"ERROR: --to must be between 1 and 8 (got {args.to_step})")
        sys.exit(1)
    ...
    step_range = f"steps {args.from_step}-{args.to_step}" if args.from_step != 1 or args.to_step != 8 else "all steps"
