...
def facts(slug: str) -> dict:
    """The four facts -- or Unreconciled, and then no facts at all.

    REFUSAL, NOT A WARNING. index.html is hand-edited; nothing regenerates it.
    What a person copies into it comes from line() below, and issues.html is
    generated by issues_page.py from this same function. So "the homepage may
    not be generated while a slug is unreconciled" can only mean one thing
    here: this derivation raises, line() raises, issues_page refuses to write,
    and the audit that polices the hand-edited index goes red. A card that
    says "not revised" about a page readers have demonstrably seen change is
    the failure this refusal exists to make impossible to derive.
    """
    u = I.unreconciled(slug)
    if u:
        raise I.Unreconciled(
            "%s. No provenance line can be derived: it would say \"not revised\" or "
            "name a date for bytes readers never had. What would have to change: a "
            "sign-off (%s) carrying %s, or the site returning to %s."
            % (I.describe_unreconciled(u), "/".join(I.SIGNOFF_ACTIONS),
               str(u["observed_sha"])[:8],
               str(u["signoff_sha"])[:8] if u["signoff_sha"] else "signed-off bytes"))
