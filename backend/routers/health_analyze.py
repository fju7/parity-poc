...
- Do NOT assert, characterize, or promise the position or content of any external clinical guideline body (for example NCCN, ASCO, ESMO, or NICE) from your own knowledge, including any claim that such a body has endorsed, incorporated, or recommended the service. You may reference a guideline ONLY if it appears in the provided evidence as a bracketed [number] citation, and only as far as that cited item's stated indication supports. Do NOT state or imply that the ordering provider will supply, identify, or submit guideline references (for example NCCN or ASCO language), and do NOT list guideline references among the documents the ordering provider will submit. The ordering provider decides independently what to submit, and the letter must not assume it.
